...
type ChatProps = {
  ...
  logoSrc?: string;
  ragUrl?: string;
  /** volver al home cuando venza la inactividad */
  onTimeout?: () => void;
};
...
Si no hay buenas opciones, usa {"gancho":"¿Quieres profundizar en algo?","opciones":[]} y NO agregues saltos de línea dentro del JSON.`;
  ...
  /** ================== INACTIVIDAD (cierre de sesión y volver al home) ================== */
  const INACTIVITY_MS = Number(import.meta.env.VITE_IDLE_MS ?? 10_000);
  ...
  // Listeners de actividad + visibilidad
  useEffect(() => {
    const activity = () => resetInactivity();
    const events: (keyof WindowEventMap)[] = ['mousemove', 'keydown', 'click', 'touchstart', 'scroll'];

    events.forEach((ev) => window.addEventListener(ev, activity, { passive: true } as any));
    const onVisibility = () => {
    ...
    return () => {
      clearInactivity();
      events.forEach((ev) => window.removeEventListener(ev, activity as any));
      document.removeEventListener('visibilitychange', onVisibility);
    };
  }, []);

  // Resetea también al llegar nuevos mensajes
  useEffect(() => {
    resetInactivity();
  }, [messages]);
  ...
  const sendMessage = async () => {
    ...
    resetInactivity(); // actividad del usuario
